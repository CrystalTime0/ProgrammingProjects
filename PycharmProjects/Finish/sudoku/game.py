...
class Game:
    def __init__(self, width, height, rows, cols, square, win):
        ...
        self.grid = self.sudoku_generator.generate_sudoku()  # [[0 for _ in range(10)] for _ in range(10)]
        self.solved_grid = self.sudoku_generator.solved_grid
        self.width = width
        self.height = height
        self.rows = rows
        self.cols = cols
    ...
    def draw_grid(self):
        self.win.fill(WHITE)
        if self.selected:
            pygame.draw.rect(self.win, SELECT_BLUE, (
                self.square * (self.selected[0]), self.square * (self.selected[1]), self.square, self.square)
                )
        for row in range(self.rows):
            for col in range(self.cols):
                if self.grid[row][col] != 0:
                    if (row, col) in self.invalid_cases:
                        self.win.blit(self.font.render(str(self.grid[row][col]), 1, BLACK),
                                      (row * self.square + X_DIGIT_MARGIN, col * self.square + Y_DIGIT_MARGIN))
                    else:
                        self.win.blit(self.font.render(str(self.grid[row][col]), 1, DIGIT_BLUE),
                                      (row * self.square + X_DIGIT_MARGIN, col * self.square + Y_DIGIT_MARGIN))
                else:
                    self.win.blit(self.font.render("", 1, BLACK),
                                  (row * self.square + X_DIGIT_MARGIN, col * self.square + Y_DIGIT_MARGIN))
        for row in range(self.rows):
